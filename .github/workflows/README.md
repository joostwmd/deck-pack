...
| ---------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`pull-request-ci.yml`**                | Pull request → `staging` or `main`    | **No deploy.** Lint, typecheck, monorepo build, validate API Dockerfile (no push). Final job **Staging merge gate** aggregates those checks; the **`staging`** ruleset requires it (plus branch up to date with `staging`) before merge.                                                    |
