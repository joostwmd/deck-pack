 ...
 */
function headersWithBearerCookie(headers: Headers, betterAuthUrl: string): Headers {
  ...
  const sessionCookie = `${getSessionCookieName(betterAuthUrl)}=${token}`;
...
export interface InvitationPort {
  createInvitation(input: CreateInvitationInput): Promise<CreateInvitationViaAuthResult>;
}

export type BetterAuthInvitationPortDeps = {
  /** Better Auth `createInvitation` API (plugin endpoint). */
  createInvitation: CreateInvitationApi["createInvitation"];
  betterAuthUrl: string;
};

/**
 * Creates an org invitation through Better Auth so `sendInvitationEmail` runs (Resend).
 * Auth API is injected so this package does not import `@deck-pack/auth/server`.
 */
export class BetterAuthInvitationPort implements InvitationPort {
  constructor(private readonly deps: BetterAuthInvitationPortDeps) {}

  async createInvitation(input: CreateInvitationInput): Promise<CreateInvitationViaAuthResult> {
    try {
      const invitation = await this.deps.createInvitation({
        body: {
          email: input.email,
          role: input.role,
        ...
        headers: headersWithBearerCookie(input.headers, this.deps.betterAuthUrl),
      });
