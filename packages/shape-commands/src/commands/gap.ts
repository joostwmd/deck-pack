...
function getMeanAdjacentGap(sorted: ShapeSelection["shapes"], direction: GapDirection): number {
  ...
  for (let index = 1; index < sorted.length; index += 1) {
    ...
    if (direction === "horizontal") {
      gaps.push(
        current.visualBounds.left - (previous.visualBounds.left + previous.visualBounds.width),
      );
    } else {
      gaps.push(
        current.visualBounds.top - (previous.visualBounds.top + previous.visualBounds.height),
      );
    }
  }
...
export function createGapCommand(
  id: FormattingActionId,
  defaultParams: GapParams,
): FormattingCommand<GapParams> {
  return {
    id,
    ...
    createPlan: (selection, commandParams) =>
      createGapPlan(selection, resolveGapParams(defaultParams, commandParams)),
  };
}
