...
export function minShapes(minimum: number): ApplicabilityPolicy {
  return (selection) => {
    ...
    return {
      ...
      reason: minimum === 1 ? "Select at least one object" : `Select at least ${minimum} objects`,
    };
  };
}
...
export function allShapes(
  predicate: (shape: SelectedShape) => boolean,
...
export const onlyLines: ApplicabilityPolicy = allShapes((shape) => shape.capabilities.isLine, {
  applicable: false,
