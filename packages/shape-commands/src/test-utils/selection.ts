...
export function selectionOf(shapes: SelectedShape[], slideId = "slide-1"): ShapeSelection {
  return {
    slideId,
...
export function selectionFromBounds(
  ...
  return selectionOf(
    entries.map((entry, index) => createShape(entry.id, entry, { selectionIndex: index })),
  );
}
