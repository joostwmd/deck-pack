import type { FormattingActionId, FormattingCommand, ShapeSelection, SwapAnchor, SwapParams } from "../types";
import { composePolicies, evaluateApplicability, exactShapes, supportsBoundsMutation } from "../policies";
import { getVisualCenter, moveVisualCenterTo, setVisualLeft, setVisualTop } from "../geometry/bounds";
...
function createSwapPlan(selection: ShapeSelection, anchor: SwapAnchor) {
  ...
  switch (anchor) {
    ...
    case "top-left":
      ...
      break;
    case "top-right":
      firstNext = setVisualTop(setVisualLeft(first!.rawBounds, secondRight - firstBounds.width), secondTop);
      secondNext = setVisualTop(setVisualLeft(second!.rawBounds, firstRight - secondBounds.width), firstTop);
      break;
    case "bottom-left":
      firstNext = setVisualTop(setVisualLeft(first!.rawBounds, secondLeft), secondBottom - firstBounds.height);
      secondNext = setVisualTop(setVisualLeft(second!.rawBounds, firstLeft), firstBottom - secondBounds.height);
      break;
    case "bottom-right":
...
export function createSwapCommand(id: FormattingActionId, defaultParams: SwapParams): FormattingCommand<SwapParams> {
  return {
    id,
...
export const swapBottomLeftCommand = createSwapCommand("swap-bottom-left", { anchor: "bottom-left" });
export const swapBottomRightCommand = createSwapCommand("swap-bottom-right", { anchor: "bottom-right" });
