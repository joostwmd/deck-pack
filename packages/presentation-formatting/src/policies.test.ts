...
import { createShape, selectionOf } from "./test-utils/selection";

describe("policies", () => {
  it("explains why swap is unavailable for three shapes", () => {
    expect(exactShapes(2)(selectionOf([createShape("a", { left: 0, top: 0, width: 10, height: 10 }), createShape("b", { left: 20, top: 0, width: 10, height: 10 }), createShape("c", { left: 40, top: 0, width: 10, height: 10 })]))).toEqual({
      applicable: false,
  ...
  it("requires a minimum number of shapes", () => {
    expect(minShapes(2)(selectionOf([createShape("a", { left: 0, top: 0, width: 10, height: 10 })]))).toEqual({
      applicable: false,
  ...
  it("short-circuits composed policies in order", () => {
    ...
    expect(policy(selectionOf([createShape("a", { left: 0, top: 0, width: 10, height: 10 })]))).toEqual({
      applicable: false,
      code: "min-shape-count",
      reason: "Select at least 2 objects",
    });
  ...
  it("rejects unsupported resize targets", () => {
    const line = createShape("line", { left: 0, top: 0, width: 100, height: 0 }, { isLine: true, supportsResize: false });
