...
export function getVisualBounds(raw: RawBounds): VisualBounds {
  ...
  const visualWidth = raw.width * Math.abs(Math.cos(radians)) + raw.height * Math.abs(Math.sin(radians));
  const visualHeight = raw.height * Math.abs(Math.cos(radians)) + raw.width * Math.abs(Math.sin(radians));
...
export function rawBoundsToMutation(
  ...
  const mutation: { shapeId: string; left?: number; top?: number; width?: number; height?: number } = {
    shapeId,
  };
