...
describe("match size commands", () => {
  it("matches width to the first selected shape", () => {
    ...
    expect(matchWidthCommand.createPlan(selection, undefined)).toEqual([
      { shapeId: "other", width: 50 },
    ]);
  });
});
...
describe("stack commands", () => {
  it("stacks shapes vertically with zero gap", () => {
    ...
    expect(stackVerticalCommand.createPlan(selection, undefined)).toEqual([
      { shapeId: "b", top: 20 },
    ]);
  });
  ...
  it("stacks shapes upward from the bottom anchor", () => {
    ...
    expect(stackBottomCommand.createPlan(selection, undefined)).toEqual([
      { shapeId: "a", top: 20 },
    ]);
  });
  ...
  it("stacks shapes leftward from the right anchor", () => {
    ...
    expect(stackRightCommand.createPlan(selection, undefined)).toEqual([
      { shapeId: "a", left: 20 },
    ]);
  });
});
...
describe("gap commands", () => {
  ...
  it("uses baked-in defaults when gap params are omitted", () => {
    ...
    expect(gapIncreaseHorizontalCommand.evaluate(selection, undefined)).toEqual({
      applicable: true,
    });
...
describe("text commands", () => {
  it("sets autofit mode on text-capable shapes", () => {
    ...
    expect(textWrapOnCommand.createPlan(selection, undefined)).toEqual([
      { shapeId: "a", wordWrap: true },
    ]);
  });
...
describe("rectify lines command", () => {
  it("sets the minor dimension to zero", () => {
    const selection = selectionFromBounds([{ id: "line", left: 0, top: 0, width: 100, height: 2 }]);
    selection.shapes[0] = createShape(
      "line",
      { left: 0, top: 0, width: 100, height: 2 },
      { isLine: true },
    );

    expect(rectifyLinesCommand.createPlan(selection, undefined)).toEqual([
      { shapeId: "line", height: 0 },
    ]);
  });
});
