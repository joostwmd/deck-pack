...
import { defaultAspectRatio } from "@deck-pack/ui/components/gallery/gallery-catalog-fields";
import { GalleryDetailView } from "@deck-pack/ui/components/gallery/gallery-detail-view";
import {
  galleryKeys,
  uploadGalleryFile,
  type GalleryFileRef,
  type GalleryItemDetail,
  type GalleryUploadRole,
} from "@deck-pack/hooks/gallery";
...
function useGalleryFileSlot(args: {
  itemId: string;
...
function GalleryDetailForm({
  ...
  const [aspectRatio, setAspectRatio] = useState<SlideAspectRatio>(
    () => (item.slide?.aspectRatio ?? defaultAspectRatio()) as SlideAspectRatio,
  );
  ...
  const invalidate = () => {
    void queryClient.invalidateQueries({ queryKey: galleryKeys.detail(itemId) });
  ...
  const applyServerDetail = (detail: GalleryItemDetail) => {
    ...
    setAspectRatio((detail.slide?.aspectRatio ?? defaultAspectRatio()) as SlideAspectRatio);
    setAliasesEpoch((epoch) => epoch + 1);
    void queryClient.setQueryData(galleryKeys.detail(itemId), detail);
  };
  ...
  const publishMutation = useMutation({
    mutationFn: () => gallery.publish({ id: itemId }),
    onSuccess: (detail) => {
      void queryClient.setQueryData(galleryKeys.detail(itemId), detail);
      invalidate();
  ...
  const unpublishMutation = useMutation({
    mutationFn: () => gallery.unpublish({ id: itemId }),
    onSuccess: (detail) => {
      void queryClient.setQueryData(galleryKeys.detail(itemId), detail);
      invalidate();
  ...
  const archiveMutation = useMutation({
    mutationFn: () => gallery.archive({ id: itemId }),
    onSuccess: (detail) => {
  ...
  const svgSlot = useGalleryFileSlot({
    itemId,
    role: "svg",
    label: "SVG",
  ...
  const presentationSlot = useGalleryFileSlot({
    itemId,
    role: "presentation",
  ...
  const thumbnailSlot = useGalleryFileSlot({
    itemId,
    role: "thumbnail",
    label: "Thumbnail",
  ...
  const rectSlot = useGalleryFileSlot({
    itemId,
    role: "rectangle",
  ...
  const squareSlot = useGalleryFileSlot({
    itemId,
    role: "square",
  ...
  const circleSlot = useGalleryFileSlot({
    itemId,
    role: "circle",
...
export function GalleryDetailPanel({
  ...
  const detailQuery = useQuery({
    queryKey: galleryKeys.detail(itemId),
    queryFn: () => gallery.get({ id: itemId }),
  });
