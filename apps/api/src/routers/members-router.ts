import { z } from "zod";

import {
  AcceptInvitation,
  AcceptPendingSeat,
  AddMember,
  CancelInvitation,
  GetInvitationPreview,
  GetOrganizationProfile,
  GetPendingJoin,
  ListMembers,
  RemoveMember,
  UpdateMemberRole,
} from "@deck-pack/members";
import {
  acceptInvitationInputSchema,
  acceptPendingSeatInputSchema,
  addMemberInputSchema,
  cancelInvitationInputSchema,
  invitationPreviewSchema,
  joinResultSchema,
  memberListEntrySchema,
  pendingJoinSchema,
  removeMemberInputSchema,
  updateMemberRoleInputSchema,
} from "@deck-pack/members/schemas";

import type { AppContainer } from "../container";
import { requireActiveOrganizationId } from "../trpc/guards/assertions/require-active-organization-id";
import { requirePermission } from "../trpc/guards/middleware/require-permission";
import {
  organizationMemberProcedure,
  protectedProcedure,
  teamWorkspaceProcedure,
} from "../trpc/procedures";
import { router } from "../trpc/init";

const listMembersProcedure = teamWorkspaceProcedure.use(requirePermission({ member: ["update"] }));
const addMemberProcedure = teamWorkspaceProcedure.use(requirePermission({ member: ["create"] }));
const updateMemberRoleProcedure = teamWorkspaceProcedure.use(
  requirePermission({ member: ["update"] }),
);
const removeMemberProcedure = teamWorkspaceProcedure.use(requirePermission({ member: ["delete"] }));
const cancelInvitationProcedure = teamWorkspaceProcedure.use(
  requirePermission({ invitation: ["cancel"] }),
);

export function membersRouter(container: AppContainer) {
  return router({
    list: listMembersProcedure.output(z.array(memberListEntrySchema)).query(async ({ ctx }) => {
      const organizationId = requireActiveOrganizationId(ctx);
      return new ListMembers(container.membersRepository).execute({ organizationId });
    }),

    add: addMemberProcedure
      .input(addMemberInputSchema)
      .output(
        z.discriminatedUnion("kind", [
          z.object({ kind: z.literal("member"), memberId: z.string() }),
          z.object({ kind: z.literal("invitation"), invitationId: z.string() }),
        ]),
      )
      .mutation(async ({ ctx, input }) => {
        const organizationId = requireActiveOrganizationId(ctx);
        return new AddMember(container.membersRepository, container.invitationPort).execute({
          organizationId,
          email: input.email,
          role: input.role,
          assignSeat: input.assignSeat,
          inviterId: ctx.session!.user.id,
          headers: ctx.headers,
        });
      }),

    updateRole: updateMemberRoleProcedure
      .input(updateMemberRoleInputSchema)
      .output(z.object({ memberId: z.string() }))
      .mutation(async ({ ctx, input }) => {
        const organizationId = requireActiveOrganizationId(ctx);
        return new UpdateMemberRole(container.membersRepository).execute({
          organizationId,
          memberId: input.memberId,
          role: input.role,
        });
      }),

    remove: removeMemberProcedure
      .input(removeMemberInputSchema)
      .output(z.object({ memberId: z.string() }))
      .mutation(async ({ ctx, input }) => {
        const organizationId = requireActiveOrganizationId(ctx);
        return new RemoveMember(container.membersRepository).execute({
          organizationId,
          memberId: input.memberId,
        });
      }),

    cancelInvitation: cancelInvitationProcedure
      .input(cancelInvitationInputSchema)
      .output(z.object({ invitationId: z.string() }))
      .mutation(async ({ ctx, input }) => {
        const organizationId = requireActiveOrganizationId(ctx);
        return new CancelInvitation(container.membersRepository).execute({
          organizationId,
          invitationId: input.invitationId,
        });
      }),

    getOrganizationProfile: organizationMemberProcedure
      .output(
        z.object({
          type: z.enum(["individual", "team"]).nullable(),
          workspace: z.enum(["solo", "team"]).nullable(),
          plan: z
            .object({
              id: z.string(),
              name: z.string(),
              slug: z.string(),
              quantity: z.number().int(),
            })
            .nullable(),
        }),
      )
      .query(async ({ ctx }) => {
        const organizationId = requireActiveOrganizationId(ctx);
        return new GetOrganizationProfile(container.membersRepository).execute({
          organizationId,
        });
      }),

    getInvitationPreview: protectedProcedure
      .input(z.object({ invitationId: z.string().trim().min(1) }))
      .output(invitationPreviewSchema)
      .query(async ({ ctx, input }) => {
        return new GetInvitationPreview(container.membersRepository).execute({
          invitationId: input.invitationId,
          userId: ctx.session!.user.id,
          userEmail: ctx.session!.user.email,
        });
      }),

    acceptInvitation: protectedProcedure
      .input(acceptInvitationInputSchema)
      .output(joinResultSchema)
      .mutation(async ({ ctx, input }) => {
        return new AcceptInvitation(container.membersRepository).execute({
          invitationId: input.invitationId,
          userId: ctx.session!.user.id,
          sessionId: ctx.session!.session.id,
          confirmReplace: input.confirmReplace,
        });
      }),

    getPendingJoin: protectedProcedure.output(pendingJoinSchema).query(async ({ ctx }) => {
      return new GetPendingJoin(container.membersRepository).execute({
        userId: ctx.session!.user.id,
        userEmail: ctx.session!.user.email,
      });
    }),

    acceptPendingSeat: protectedProcedure
      .input(acceptPendingSeatInputSchema)
      .output(joinResultSchema)
      .mutation(async ({ ctx, input }) => {
        return new AcceptPendingSeat(container.membersRepository).execute({
          userId: ctx.session!.user.id,
          userEmail: ctx.session!.user.email,
          sessionId: ctx.session!.session.id,
          confirmReplace: input.confirmReplace,
        });
      }),
  });
}
