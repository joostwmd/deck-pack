...
import type { BrandProfileConfiguration, CheckFinding, CheckResult } from "@deck-pack/presentation-check";
import { DEFAULT_BRAND_PROFILE_CONFIGURATION, runPresentationCheck } from "@deck-pack/presentation-check";
...
export function filterFindings(
  ...
  return result.findings.filter((finding) => {
    ...
    if (search && !`${finding.message} ${finding.actual} ${finding.expected}`
      .toLowerCase()
...
export function getSafeFixableFindings(findings: CheckFinding[]): CheckFinding[] {
  return findings.filter(
    (finding) => finding.suggestedFix?.safe && finding.fixMode !== "none",
  );
}
