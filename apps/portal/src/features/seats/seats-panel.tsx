...
export function SeatsPanel() {
  ...
  const capacityQuery = useQuery(trpc.seats.capacity.queryOptions());
  const seatsQuery = useQuery(trpc.seats.list.queryOptions());

  const assignMutation = useMutation(
    trpc.seats.assign.mutationOptions({
      onSuccess: async () => {
        toast.success("Seat assigned");
        setAssignOpen(false);
        setEmail("");
        await queryClient.invalidateQueries({ queryKey: trpc.seats.list.queryKey() });
        await queryClient.invalidateQueries({ queryKey: trpc.seats.capacity.queryKey() });
      },
  ...
  const revokeMutation = useMutation(
    trpc.seats.revoke.mutationOptions({
      onSuccess: async () => {
                    ...
                    <TableCell className="text-right">
                      <Can permissions={{ seat: ["assign"] }}>
                        <Button
                          ...
                          onClick={() => revokeMutation.mutate({ seatId: seat.seatId })}
                        >
                          Revoke
                        </Button>
                      </Can>
